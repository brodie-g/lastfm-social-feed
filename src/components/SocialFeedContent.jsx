import SocialFeedTrackList from "./SocialFeedTrackList.jsx";
...
export default class SocialFeedContent extends React.Component {
    ...
    componentDidMount() {
        ...
        lastfm.fetchFriends(username, (friends) => {
            ...
            if (storage.getLimitConcurrentRequests()) {
                eachLimit(friends, 3, this.fetchPlaysForFriend.bind(this));
            } else {
                each(friends, this.fetchPlaysForFriend.bind(this));
            }
        });
    }
    ...
    fetchPlaysForFriend(friend, callback) {
        if (!this._isMounted) {
            return callback('unmounted');
        }

        //
        lastfm.fetchRecentTracks(friend, (response) => {
            this.handleRecentTracksReceived(response);
            return callback();
        });
    ...
    handleRecentTracksReceived(user, recentTrackData) {
        if (recentTrackData) {
            this.setState((state) => {
                state.data = state.data.concat([
                    {
    ...
    render() {
        ...
        if (!storage.getWaitUntilFetched() || this.state.done) {
            if (this.state.done && !this.state.data.length) {
                ...
                childComponent = <SocialFeedTrackList data={this.state.data} fetched={this.state.done}/>;
            }
        } else {
